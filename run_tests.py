...
def test(file: str, process: subprocess.Popen,
         pad_length: int, is_kill: bool, start: int):
    global successes
    global failures
    global errors
    global crashes
    ...
    if is_kill:
        print("\x1b[35mSIGINT\x1b[0m", end=" ")
    elif exit_code != 0:
        crashes += 1
    ...
    with open(file.replace(".cog", ".log"), "wb") as f:
        f.write(out)
    print(f" (time: {(time.perf_counter_ns() - start)/1e9:.3g}s)", end="")
...
try:
    while any(not q for p, q in file2proc.values()):
        for f, (p, q) in file2proc.items():
            if not q:
                file2proc[f][1] = test(f, p, pl, False, start)
except KeyboardInterrupt:
    for f, (p, q) in file2proc.items():
        if not q:
            p.kill()
            test(f, p, pl, True, start)
